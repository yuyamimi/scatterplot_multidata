				...
				if(barraylist != null) {
					...
					for(int j = 0; j < barraylist.size(); j++) {
						if(numclusters >= 2) {
							...
							if(Math.abs((double)pid - barraylist.get(j)[6]) < 1.0e-10) {
								gl2.glVertex3d(barraylist.get(j)[0], barraylist.get(j)[1], 0.0);
								gl2.glVertex3d(barraylist.get(j)[3], barraylist.get(j)[4], 0.0);
								//System.out.println("barraylist [" + j + "]: " + barraylist.get(j)[0] + ", [1]: " + barraylist.get(j)[1] + ", [3]: " + barraylist.get(j)[3] + ", [4]: " + barraylist.get(j)[4]);
							}
					}
				gl2.glEnd();
				}
