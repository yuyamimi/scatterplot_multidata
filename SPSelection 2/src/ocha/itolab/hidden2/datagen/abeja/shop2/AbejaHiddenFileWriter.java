...
public class AbejaHiddenFileWriter {
	...
	public static void write(DataSet data, String filename) {		
		...
		for(int i = 0; i < data.days.size(); i++) {
			...
			line += ("," + od.revenue + "," + od.visitors + "," + od.conversion);
	...
	static void writeTopLines() {
		...
		println("Explain,Explain,Explain,Explain,Explain,Objective,Objective,Objective,Objective,Objective,Objective,Objective,Objective,Category,Category,Category,Category,Name");
		println("MinTemp,MaxTemp,SumRain,SumSunTime,MaxWind,Revenue,Visitor,Conversion,Transaction,RevTran,RevItem,ItemTran,RevVisit,Holiday,BadWeather,Month,ShopName,Date");
	}
